...
int32_t CRoomMgt::GetAllRoomByServerID(vector<CRoom *> &arrRoomInServer,const ServerID nServerID)
{
	RoomPool::CIndex *pIndex = m_sRoomPool.First();

	while(NULL != pIndex)
	{
		if(pIndex->ObjectPtr()->GetRoomServerID() == nServerID)
		{
			arrRoomInServer.push_back(pIndex->ObjectPtr());
		}
		pIndex = pIndex->Next();
	}
